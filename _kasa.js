...
const printLiveDevicesDetails = () => {
  console.log('Currently ' + devices.length + ' devices');
  devices.forEach(deviceObject => {
    const device = deviceObject.device;
    if (device) {
      console.log(deviceObject.localAlias, device.alias, device.type, device.mac, );
    }
  });
}

//setInterval(printLiveDevicesDetails, 5000);
